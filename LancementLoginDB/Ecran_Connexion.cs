...
namespace LancementLoginDB
{
  public partial class UserAccountForm : Form
  {
    ...
    private void UserLoginButton_Click(object sender, EventArgs e)
    {
      ...
      {
        //CREATION TEMP USER
        UserSignIn t_user = new();
        t_user.CheckUsername(this.UserIdTextBox.Text);
        t_user.CheckPassword(this.UserPsswTextBox.Text);

        //CONNEXION POUR L UTILISATEUR
        DBSignUpSignIn t_log = new();
        t_log.UserLogIn(t_user.getUID(), t_user.getPSWD());
      }
      catch (UserSignInException exc)
      {
        MessageBox.Show(exc.message, "ERREUR CONNEXION", MessageBoxButtons.OK, MessageBoxIcon.Information);
        switch (exc.errorCode)
        {
          case 1:
          case 2:
